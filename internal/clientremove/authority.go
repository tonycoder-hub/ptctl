...
// operation. Reading the live journal or retained tombstone is local and
// read-only; it never contacts the downloader.
...
// terminal keep-data removal. It is historical evidence and does not by itself
// establish current queue absence or attribute current absence to the request.
...
// client-removal chain was read from the bound target-root journal or exact
...
func VerifyCompletion(ctx context.Context, options CompletionProofOptions) (*VerifiedCompletion, CompletionObservation, error) {
	if options.TargetRoot == "" || !canonicalPlanID(options.ExpectedPlanID) ||
		...
		return nil, CompletionObservation{}, fmt.Errorf("%w: client removal completion selector is invalid", ErrPolicy)
	}
	...
	if state.Pending != "" || state.Completion == nil || state.CompletionID == "" || state.IntentID == "" ||
		...
		return nil, CompletionObservation{}, fmt.Errorf("%w: terminal client removal completion is unavailable or disagrees with the selector", ErrPolicy)
	}
	authority := &verifiedCompletionAuthority{
	...
	if !verified.Verified() {
		return nil, CompletionObservation{}, fmt.Errorf("%w: terminal client removal completion is invalid", ErrIntegrity)
	}
...
func (verified *VerifiedCompletion) Verified() bool {
	...
	if completion.ResponseID == "" {
		if authority.response != nil || authority.responseID != "" || completion.Basis == "accepted_response_then_exact_absence" {
			return false
		}
	} else {
		...
		if _, id, err := encodeResponse(*authority.response); err != nil || id != authority.responseID ||
			...
			authority.response.PlanID != authority.planID || completion.Basis == "accepted_response_then_exact_absence" && !authority.response.Complete {
			return false
		}
	}
	return authority.intent.OperationID == authority.operationID && authority.intent.PlanID == authority.planID &&
		...
		authority.attempt.UseID == plan.UseID && authority.attempt.JobID == plan.JobID && authority.attempt.FileLayoutID == plan.FileLayoutID &&
		authority.attempt.FileSnapshotID == plan.CompleteFileSnapshotID &&
		completion.UseID == plan.UseID && completion.JobID == plan.JobID &&
		completion.FinalObjectIdentity == plan.FinalObjectIdentity
}
...
func (verified *VerifiedCompletion) Observation() CompletionObservation {
	...
	if authority.retained {
		assurance = "same_invocation_bound_canonical_client_removal_retention_tombstone_read_without_current_queue_inference"
	}
	return CompletionObservation{
		...
		ObservedAtEnd:   completion.ObservedAtEnd.UTC().Format(time.RFC3339Nano), JobsExamined: completion.JobsExamined,
