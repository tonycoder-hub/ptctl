...
func (limits RetentionLimits) Validate() error {
	if limits.MaxMarkerBytes <= 0 || limits.MaxMarkerBytes > maximumMarkerBytes ||
		...
		return fmt.Errorf("%w: client removal retention limits are invalid", ErrPolicy)
	}
	return nil
}
...
type RetentionRemovalReport struct {
	...
	Responses  []fsbind.Removal `json:"responses"`
...
func newRetentionReport(options PruneOptions) RetentionReport {
	return RetentionReport{
		...
		Effect: []string{
			"read_private_client_removal_operation_state",
			"write_private_client_removal_retention_markers",
			"delete_private_client_removal_operation_state",
		},
		...
		Blockers:  []Finding{}, Issues: []Finding{}, Warnings: []string{
			"prune deletes only one explicit removal operation's owner-private journal and retains a small tombstone",
		...
		Removals: RetentionRemovalReport{Attempts: []fsbind.Removal{}, Responses: []fsbind.Removal{}},
	}
}
