...
func (*forgetInProgressError) Error() string {
	return "client removal historical evidence deletion is in progress"
}
...
// removal tombstone. A private root-level intent is durably published first
...
func Forget(ctx context.Context, options ForgetOptions) (ForgetReport, error) {
	...
	if err := options.Limits.Validate(); err != nil {
		return forgetBlocked(&report, "limits.invalid", "client removal forget limits are invalid")
	}
	if !options.Acknowledge {
		return forgetBlocked(&report, "acknowledgement.required", "historical removal tombstone deletion requires its dedicated acknowledgement")
	}
	if options.TargetRoot == "" || !canonicalPlanID(options.ExpectedPlanID) || OperationIDForPlan(options.ExpectedPlanID) != options.OperationID {
		return forgetBlocked(&report, "forget.selector_invalid", "target root, operation ID, or reviewed removal plan ID is invalid")
	}
	if parsed, err := ParseOperationID(options.OperationID.String()); err != nil || parsed != options.OperationID {
		return forgetBlocked(&report, "forget.selector_invalid", "target root, operation ID, or reviewed removal plan ID is invalid")
	}
	if err := ctx.Err(); err != nil {
		return forgetInterrupted(&report, err, "client removal forget was interrupted before target observation")
	}
	absolute, err := filepath.Abs(options.TargetRoot)
	if err != nil {
		return forgetBlocked(&report, "target.invalid_root", "the client removal target root is invalid")
	}
	target, rootInfo, err := fsbind.BindExisting(filepath.Clean(absolute))
	if err != nil {
		return mapForgetError(&report, err, "the target root cannot provide bound removal tombstone-forget semantics")
	}
	defer target.Close()
	...
	if errors.Is(markerErr, fsbind.ErrNotFound) {
		...
		if errors.Is(openErr, fsbind.ErrNotFound) {
			...
			report.Issues = append(report.Issues, Finding{Code: "forget.absent_unattributed", Message: "neither the retained removal operation nor a durable forget intent exists; prior erasure cannot be distinguished from an unknown selector"})
			return report, ErrOperationNotFound
		}
		if openErr != nil {
			return mapForgetError(&report, openErr, "the retained client removal operation could not be opened")
		}
		...
		if loadErr != nil {
			return mapForgetError(&report, loadErr, "the retained client removal tombstone could not be loaded")
		}
		if !state.IntentPresent || !state.CompletePresent || state.IntentPending || state.CompletePending {
			return forgetBlocked(&report, "operation.prune_required", "only a complete retained client removal tombstone may be forgotten")
		}
		if state.Intent.PlanID != options.ExpectedPlanID {
			return forgetBlocked(&report, "plan.id_mismatch", "the reviewed removal plan ID does not select this tombstone")
		}
		...
		if err := marker.Validate(); err != nil {
			return mapForgetError(&report, err, "the retained client removal tombstone cannot authorize forgetting")
		}
		if err := auditInitialForgetTombstone(ctx, handle, state, marker); err != nil {
			return mapForgetError(&report, err, "the client removal tombstone is not exact before forget intent publication")
		}
		preparedRaw, preparedID, prepareErr := EncodeForgetIntent(marker)
		if prepareErr != nil {
			return mapForgetError(&report, prepareErr, "the client removal forget intent cannot be encoded")
		}
		if int64(len(preparedRaw)) > options.Limits.MaxMarkerBytes {
			return mapForgetError(&report, ErrPolicy, "the client removal forget marker byte budget is insufficient")
		}
		markerID = preparedID
		populateForgetAuthority(&report, marker, markerID)
		report.Authority.State = "exact_tombstone_observed"
		report.Operation.Status, report.Operation.PhaseBefore, report.Operation.PhaseAfter = "retained", "retained_removal_completion", "retained_removal_completion"
		...
		if publishErr != nil {
			...
			return mapForgetError(&report, publishErr, "the client removal forget intent could not be durably published")
		}
	} else if markerErr != nil {
		return mapForgetError(&report, markerErr, "the client removal forget intent could not be read")
	} else if err := selectForgetMarker(marker, options, rootInfo.Identity); err != nil {
		return mapForgetError(&report, err, "the client removal forget intent does not match the explicit selector")
	} else {
		populateForgetAuthority(&report, marker, markerID)
		...
		if err := target.SyncRoot(ctx); err != nil {
			return mapForgetError(&report, err, "the visible client removal forget intent durability could not be confirmed")
		}
		...
		if freshErr != nil || !sameForgetIntent(fresh, marker) || freshID != markerID || !freshObject.Identity.Equal(rootObject.Identity) {
			if freshErr != nil {
				return mapForgetError(&report, classifyJournalError(freshErr), "the client removal forget intent could not be re-read after durability confirmation")
			}
			return mapForgetError(&report, ErrIntegrity, "the client removal forget intent changed during durability confirmation")
		}
	...
	if markerID == "" {
		...
		if encodeErr != nil {
			return mapForgetError(&report, encodeErr, "the client removal forget intent cannot be reproduced")
		}
		markerID = derivedID
	}
	populateForgetAuthority(&report, marker, markerID)
	...
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("root_intent_published"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client removal forgetting stopped after its durable root intent")
		}
	}
	...
	if handle == nil {
		...
		if parseErr != nil {
			return mapForgetError(&report, parseErr, "the client removal forget operation identity is invalid")
		}
		...
		if errors.Is(openErr, fsbind.ErrNotFound) {
			...
			case errors.Is(inspectErr, fsbind.ErrNotFound):
				if syncErr := target.SyncRoot(ctx); syncErr != nil {
					return mapForgetError(&report, syncErr, "the recovered client removal operation absence is not durable")
				}
				if _, confirmErr := target.InspectRoot(ctx, operationName); !errors.Is(confirmErr, fsbind.ErrNotFound) {
					if confirmErr == nil {
						confirmErr = fmt.Errorf("%w: the client removal operation reappeared", ErrIntegrity)
					}
					return mapForgetError(&report, confirmErr, "the recovered client removal operation absence changed")
				}
			case inspectErr != nil:
				return mapForgetError(&report, inspectErr, "the client removal operation removal state is unavailable")
			case observed.Kind != fsbind.ObjectKindDirectory || !observed.Identity.Equal(expectedOperation):
				return mapForgetError(&report, fsbind.ErrUnsafeObject, "the client removal operation residue changed identity")
			default:
				...
				if residueErr != nil {
					return mapForgetError(&report, residueErr, "the client removal empty operation residue could not be removed")
				}
			}
		} else if openErr != nil {
			return mapForgetError(&report, openErr, "the client removal operation could not be rebound for forgetting")
		} else {
			handle = &journalHandle{session: target, subtree: subtree}
			defer func() {
				if handle != nil && handle.subtree != nil {
	...
	if handle != nil {
		...
		report.recordForgetMarker(receipt)
		if ensureErr != nil {
			return mapForgetError(&report, ensureErr, "the existing client removal forget intent could not be rebound to its retained tombstone")
		}
		if ensuredID != markerID || !ensuredObject.Identity.Equal(rootObject.Identity) {
			return mapForgetError(&report, ErrIntegrity, "the existing client removal forget intent changed while its tombstone was rebound")
		}
		rootObject = ensuredObject
		if err := removeTombstoneFromForgetIntent(ctx, target, handle, marker, &report); err != nil {
			return mapForgetError(&report, err, "the retained client removal tombstone could not be completely removed")
		}
		handle = nil
	}
	...
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("operation_removed"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client removal forgetting stopped after operation removal")
		}
	}
	...
	if freshErr != nil || !sameForgetIntent(freshMarker, marker) || freshID != markerID || !freshObject.Identity.Equal(rootObject.Identity) {
		...
		if freshErr != nil {
			return mapForgetError(&report, freshErr, "the final client removal forget marker read failed")
		}
		return mapForgetError(&report, ErrIntegrity, "the final client removal forget marker changed")
	}
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("before_root_intent_remove"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client removal forgetting stopped before last-marker removal")
		}
	}
	operationName, _ := operationDirectoryName(options.OperationID)
	if _, absenceErr := target.InspectRoot(ctx, operationName); !errors.Is(absenceErr, fsbind.ErrNotFound) {
		if absenceErr == nil {
			absenceErr = fmt.Errorf("%w: the client removal operation name reappeared", ErrIntegrity)
		}
		return mapForgetError(&report, absenceErr, "the client removal operation is no longer absent before last-marker removal")
	}
	if syncErr := target.SyncRoot(ctx); syncErr != nil {
		return mapForgetError(&report, syncErr, "the client removal operation absence is not durable before last-marker removal")
	}
	if _, absenceErr := target.InspectRoot(ctx, operationName); !errors.Is(absenceErr, fsbind.ErrNotFound) {
		if absenceErr == nil {
			absenceErr = fmt.Errorf("%w: the client removal operation name reappeared", ErrIntegrity)
		}
		return mapForgetError(&report, absenceErr, "the client removal operation absence changed before last-marker removal")
	}
	...
	if removeErr != nil {
		...
		return mapForgetError(&report, removeErr, "the last client removal forget marker removal was not confirmed")
	}
	if !rootRemoval.Removed || rootRemoval.Durability != fsbind.DurabilityConfirmed {
		report.Authority.State = "last_marker_removal_durability_unconfirmed"
		report.Authority.MarkerDurable = false
		return mapForgetError(&report, fsbind.ErrDurabilityUnconfirmed, "the last client removal forget marker durability was not confirmed")
	}
	...
	report.Warnings = append(report.Warnings, "forget completed: no ptctl client removal history for this operation remains in the target root")
	return report, nil
}
...
func selectForgetMarker(marker ForgetIntent, options ForgetOptions, targetIdentity fsbind.Identity) error {
	...
	if marker.OperationID != options.OperationID || marker.TargetRootIdentity != targetIdentity.String() {
		return fmt.Errorf("%w: client removal forget marker is bound to another filesystem object", ErrIntegrity)
	}
	if marker.PlanID != options.ExpectedPlanID {
		return fmt.Errorf("%w: client removal forget selector disagrees", ErrPolicy)
	}
	return nil
}
...
func inspectForgetControl(ctx context.Context, target *fsbind.Session, operation OperationID, expectedPlanID string) (*forgetInProgressError, error) {
	if target == nil {
		return nil, fmt.Errorf("%w: client removal forget inspection authority is unavailable", ErrIntegrity)
	}
	rootName, err := ForgetRootName(operation)
	if err != nil {
		return nil, fmt.Errorf("%w: client removal forget selector is invalid", ErrPolicy)
	}
	...
	if marker.OperationID != operation || marker.TargetRootIdentity != target.Info().Identity.String() {
		return nil, fmt.Errorf("%w: client removal forget marker is bound to another operation", ErrIntegrity)
	}
	if expectedPlanID != "" && marker.PlanID != expectedPlanID {
		return nil, fmt.Errorf("%w: client removal forget marker differs from the reviewed plan", ErrPolicy)
	}
...
func applyForgetControlReport(report *Report, pending *forgetInProgressError) {
	...
	report.Effect = "read_private_client_removal_forget_intent"
	phase := "forget_intent_staging"
	if pending.rootDurable {
		phase = "forget_intent_recorded"
	}
	report.Operation = OperationReport{ID: marker.OperationID.String(), Status: "forgetting", PhaseBefore: "retained_removal_completion", PhaseAfter: phase, Resumable: false}
	plan := marker.RetentionIntent.Intent.Plan
	report.Plan = PlanReport{ID: marker.PlanID, ExpectedID: report.Plan.ExpectedID, Matches: report.Plan.ExpectedID == "" || report.Plan.ExpectedID == marker.PlanID, Data: plan}
	report.Retention = JournalRetentionReport{State: "forgetting", IntentMarkerID: marker.RetentionIntentMarkerID.String(),
		CompletionMarkerID: marker.RetentionCompleteMarkerID.String(), IntentDurable: true, CompletionDurable: true,
		HistoricalTerminalEvidence: true}
	report.Final = materialize.FinalObservation{OperationID: plan.MaterializeOperationID, MaterializePlanID: plan.MaterializePlanID,
		...
		ManifestFiles: plan.ManifestFiles, ContentBytes: plan.ContentBytes, AuthorityBasis: "historical_forget_basis", Assurance: "current_final_not_observed"}
	report.Outcome = OutcomeBlocked
	report.Blockers = append(report.Blockers, "client_removal.forget_required")
	markerWarning := "client removal historical evidence deletion has a private staged intent; only explicit forget may publish or recover it"
	if pending.rootDurable {
		markerWarning = "client removal historical evidence is being forgotten under an explicit root-level recovery marker; read-only status does not reassert its durability"
	}
...
func populateForgetAuthority(report *ForgetReport, marker ForgetIntent, markerID ForgetMarkerID) {
	...
	report.Proof = RetentionProofReport{Basis: marker.RetentionIntent.Basis, IntentID: marker.RetentionIntent.IntentID.String(),
		CompletionID: marker.RetentionIntent.CompletionID.String(), AttemptsRecorded: len(marker.RetentionIntent.Attempts),
		ResponsesRecorded: len(marker.RetentionIntent.Responses), HistoricalAuthority: true,
		Assurance: "exact_removal_retention_tombstone_copied_into_durable_forget_intent"}
}
...
func auditInitialForgetTombstone(ctx context.Context, handle *journalHandle, state retentionState, marker ForgetIntent) error {
	if handle == nil || handle.subtree == nil || !state.IntentPresent || !state.CompletePresent || state.IntentPending || state.CompletePending ||
		...
		return fmt.Errorf("%w: retained client removal tombstone disagrees with forget intent", ErrIntegrity)
	}
	...
	if err != nil || !listing.Complete {
		...
		return fmt.Errorf("%w: client removal forget preflight inventory is incomplete", ErrIntegrity)
	}
	...
	for _, entry := range listing.Entries {
		...
		case pendingName:
			if entry.Kind != string(fsbind.ObjectKindRegular) {
				return fmt.Errorf("%w: client removal forget pending marker is unsafe", ErrIntegrity)
			}
			if _, verifyErr := handle.verifyNamedBytes(ctx, []string{retentionDirectoryName, pendingName}, raw); verifyErr != nil {
				return verifyErr
			}
		default:
			return fmt.Errorf("%w: client removal forget preflight contains an unexpected object", ErrIntegrity)
		}
	}
	if !seenIntent || !seenComplete {
		return fmt.Errorf("%w: retained client removal tombstone is incomplete", ErrIntegrity)
	}
...
func removeTombstoneFromForgetIntent(ctx context.Context, target *fsbind.Session, handle *journalHandle, marker ForgetIntent, report *ForgetReport) error {
	...
	if err != nil || handle == nil || handle.subtree == nil || !handle.subtree.Identity().Equal(expectedOperation) {
		return fmt.Errorf("%w: client removal operation identity changed", ErrIntegrity)
	}
	...
	if directoryErr != nil || directory.Kind != fsbind.ObjectKindDirectory {
		...
		return fmt.Errorf("%w: client removal retention directory is unsafe", ErrIntegrity)
	}
	...
	if err != nil || !listing.Complete {
		...
		return fmt.Errorf("%w: client removal forget recovery inventory is incomplete", ErrIntegrity)
	}
	for _, entry := range listing.Entries {
		if entry.Kind != string(fsbind.ObjectKindRegular) || entry.Name != retentionIntentName && entry.Name != retentionCompleteName {
			return fmt.Errorf("%w: client removal forget recovery contains an unexpected object", ErrIntegrity)
		}
	}
	...
	if completeErr != nil || intentErr != nil || completeID != marker.RetentionCompleteMarkerID || intentID != marker.RetentionIntentMarkerID {
		return fmt.Errorf("%w: retained client removal tombstone cannot be reproduced", ErrIntegrity)
	}
	for _, selected := range []struct {
		...
		if inspectErr != nil || object.Kind != fsbind.ObjectKindRegular || object.SizeBytes != int64(len(selected.raw)) {
			...
			return fmt.Errorf("%w: retained client removal marker is unsafe", ErrIntegrity)
		}
		...
		if verifyErr != nil || !identity.Equal(object.Identity) {
			...
			return fmt.Errorf("%w: retained client removal marker identity changed", ErrIntegrity)
		}
	...
	if err != nil || !after.Complete || len(after.Entries) != 0 {
		...
		return fmt.Errorf("%w: client removal retention directory is not empty after marker removal", ErrIntegrity)
	}
...
func removeForgetOperationRoot(ctx context.Context, target *fsbind.Session, handle *journalHandle, report *ForgetReport) error {
	...
	if err != nil || !root.Complete || len(root.Entries) != 1 || root.Entries[0].Name != operationLockEntryName || root.Entries[0].Kind != string(fsbind.ObjectKindRegular) {
		...
		return fmt.Errorf("%w: client removal operation root is not an exact lock-only namespace", ErrIntegrity)
	}
