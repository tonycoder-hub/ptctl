...
func TestExistingJobStopUsesExactRequestForBothProtocols(t *testing.T) {
	for _, modern := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			receipt, err := session.Stop(context.Background(), downloader.ExistingJobMutationRequest{JobKey: testHash})
			...
			if err != nil || !receipt.Complete || receipt.Effect != downloader.StopEffect || receipt.RequestsAttempted != 1 ||
				...
				receipt.RequestBytes != int64(len(captured)) || receipt.RequestID <= 0 || session.RequestsMade() != 3 ||
				wireRequests != 3 || actionRequests != 1 {
				t.Fatalf("receipt=%#v session=%d wire=%d actions=%d err=%v", receipt, session.RequestsMade(), wireRequests, actionRequests, err)
			}
			expected, err := downloader.MarshalExistingJobStopRequest(descriptor, testHash, receipt.RequestID)
			if err != nil || !bytes.Equal(captured, expected) {
				t.Fatalf("wire request differs:\n got %s\nwant %s\nerr=%v", captured, expected, err)
			}
			second, secondErr := session.Stop(context.Background(), downloader.ExistingJobMutationRequest{JobKey: testHash})
...
func TestExistingJobStopNeverReplaysExpiredCSRFOrLeaksSelector(t *testing.T) {
