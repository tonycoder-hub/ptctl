...
func (s *readSession) ReadExistingJobRemovalDescriptor(ctx context.Context) (downloader.ExistingJobRemovalDescriptor, error) {
	...
	if s.closed {
		s.mu.Unlock()
		return downloader.ExistingJobRemovalDescriptor{}, fmt.Errorf("Transmission read session is closed")
	}
	if s.removalDescriptorAttempted {
		s.mu.Unlock()
		return downloader.ExistingJobRemovalDescriptor{}, fmt.Errorf("Transmission removal descriptor was already requested")
	}
	s.removalDescriptorAttempted = true
	valueProtocol := s.protocol
	s.mu.Unlock()
	descriptor := downloader.ExistingJobRemovalDescriptor{
		Driver: downloader.DriverTransmission, RemoveRouteID: "transmission.torrent.remove_keep_data.v1",
	}
	...
	default:
		return downloader.ExistingJobRemovalDescriptor{}, fmt.Errorf("Transmission removal protocol is unsupported")
	}
...
func (s *readSession) RemoveKeepData(ctx context.Context, request downloader.ExistingJobMutationRequest) (downloader.ExistingJobMutationReceipt, error) {
	...
	if closed || attempted || descriptor.Validate() != nil {
		return fail("removal_descriptor_unavailable", fmt.Errorf("Transmission removal session is unavailable"))
	}
	body, err := downloader.MarshalExistingJobRemovalRequest(descriptor, request.JobKey, id)
	if err != nil {
		return fail("job_locator_invalid", fmt.Errorf("Transmission removal selector is invalid"))
	}
	s.mu.Lock()
	if s.closed || s.removeAttempted || s.removalDescriptor != descriptor || s.nextID != id {
		s.mu.Unlock()
		return fail("session_unavailable", fmt.Errorf("Transmission removal session is unavailable"))
	}
	...
	if requestDelta < 0 || requestDelta > 1 {
		return fail("session_unavailable", fmt.Errorf("Transmission removal request count is invalid"))
	}
	if requestErr != nil {
	...
	if response.status != http.StatusOK {
		return fail("http_rejected", fmt.Errorf("Transmission rejected the removal request"))
	}
	result, err := decodeRPCResult(response.body, removalProtocol(descriptor), id)
	if err != nil {
		return fail("response_invalid", err)
	}
	fields, err := decodeObject(result, 1)
	if err != nil || len(fields) != 0 {
		return fail("response_invalid", fmt.Errorf("Transmission removal response was not accepted"))
	}
	if err := ctx.Err(); err != nil {
		return fail("context_cancelled", err)
	}
...
func removalProtocol(descriptor downloader.ExistingJobRemovalDescriptor) protocol {
