package transmission

import (
	"context"
	"fmt"
	"net/http"
	"time"

	"github.com/tonycoder-hub/ptctl/internal/downloader"
)

const maxControlResponseBytes = int64(64 << 10)

func (a *Adapter) OpenExistingJobMutationSession(ctx context.Context, credential downloader.Credential) (downloader.ExistingJobMutationSession, error) {
	session, err := a.openReadSession(ctx, credential)
	if err != nil {
		return nil, err
	}
	return session, nil
}

func (s *readSession) ReadExistingJobControlDescriptor(ctx context.Context) (downloader.ExistingJobControlDescriptor, error) {
	if err := ctx.Err(); err != nil {
		return downloader.ExistingJobControlDescriptor{}, err
	}
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return downloader.ExistingJobControlDescriptor{}, fmt.Errorf("Transmission read session is closed")
	}
	if s.controlAttempted {
		s.mu.Unlock()
		return downloader.ExistingJobControlDescriptor{}, fmt.Errorf("Transmission control descriptor was already requested")
	}
	s.controlAttempted = true
	valueProtocol := s.protocol
	s.mu.Unlock()

	descriptor := downloader.ExistingJobControlDescriptor{
		Driver: downloader.DriverTransmission, RecheckRouteID: "transmission.torrent.verify.v1",
		StartRouteID: "transmission.torrent.start.v1",
	}
	switch valueProtocol {
	case protocolLegacy:
		descriptor.Protocol = downloader.ControlProtocolTransmissionV5
	case protocolJSONRPC:
		descriptor.Protocol = downloader.ControlProtocolTransmissionV6
	default:
		return downloader.ExistingJobControlDescriptor{}, fmt.Errorf("Transmission control protocol is unsupported")
	}
	if err := descriptor.Validate(); err != nil {
		return downloader.ExistingJobControlDescriptor{}, err
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.closed {
		return downloader.ExistingJobControlDescriptor{}, fmt.Errorf("Transmission read session is closed")
	}
	s.controlDescriptor = descriptor
	return descriptor, nil
}

func (s *readSession) Recheck(ctx context.Context, request downloader.ExistingJobMutationRequest) (downloader.ExistingJobMutationReceipt, error) {
	return s.mutateExistingJob(ctx, downloader.ControlEffectRecheck, request)
}

func (s *readSession) Start(ctx context.Context, request downloader.ExistingJobMutationRequest) (downloader.ExistingJobMutationReceipt, error) {
	return s.mutateExistingJob(ctx, downloader.ControlEffectStart, request)
}

func (s *readSession) mutateExistingJob(ctx context.Context, effect string, request downloader.ExistingJobMutationRequest) (downloader.ExistingJobMutationReceipt, error) {
	receipt := downloader.ExistingJobMutationReceipt{Effect: effect, ObservedAtStart: time.Now().UTC()}
	fail := func(reason string, err error) (downloader.ExistingJobMutationReceipt, error) {
		receipt.StopReason = reason
		receipt.ObservedAtEnd = time.Now().UTC()
		return receipt, err
	}
	if err := ctx.Err(); err != nil {
		return fail("context_cancelled", err)
	}
	s.mu.Lock()
	descriptor, closed := s.controlDescriptor, s.closed
	id, token := s.nextID, s.csrfToken
	if !closed && descriptor.Validate() == nil {
		s.nextID++
	}
	s.mu.Unlock()
	if closed || descriptor.Driver != downloader.DriverTransmission || descriptor.Validate() != nil {
		return fail("control_descriptor_unavailable", fmt.Errorf("Transmission control descriptor is unavailable"))
	}
	body, err := downloader.MarshalExistingJobMutationRequest(descriptor, effect, request.JobKey, id)
	if err != nil {
		return fail("job_locator_invalid", fmt.Errorf("Transmission existing-job selector is invalid"))
	}
	requestsBefore := s.RequestsMade()
	response, requestErr := s.execute(ctx, body, token, maxControlResponseBytes)
	requestDelta := s.RequestsMade() - requestsBefore
	if requestDelta == 1 {
		receipt.RequestsAttempted = 1
	}
	if requestDelta < 0 || requestDelta > 1 {
		return fail("session_unavailable", fmt.Errorf("Transmission control request count is invalid"))
	}
	if requestErr != nil {
		if contextErr := ctx.Err(); contextErr != nil {
			return fail("context_cancelled", contextErr)
		}
		return fail("transport_failed", requestErr)
	}
	if response.status == http.StatusConflict {
		return fail("csrf_expired", fmt.Errorf("Transmission CSRF session expired; automatic replay is disabled"))
	}
	if response.status == http.StatusUnauthorized || response.status == http.StatusForbidden {
		return fail("http_rejected", fmt.Errorf("Transmission rejected the credentials"))
	}
	if response.status != http.StatusOK {
		return fail("http_rejected", fmt.Errorf("Transmission rejected the existing-job request"))
	}
	result, err := decodeRPCResult(response.body, protocolForDescriptor(descriptor), id)
	if err != nil {
		return fail("response_invalid", err)
	}
	fields, err := decodeObject(result, 1)
	if err != nil || len(fields) != 0 {
		return fail("response_invalid", fmt.Errorf("Transmission existing-job response was not accepted"))
	}
	if err := ctx.Err(); err != nil {
		return fail("context_cancelled", err)
	}
	receipt.Complete = true
	receipt.RequestBytes = int64(len(body))
	receipt.RequestBytesKnown = true
	receipt.RequestID = id
	receipt.ObservedAtEnd = time.Now().UTC()
	return receipt, nil
}

func protocolForDescriptor(descriptor downloader.ExistingJobControlDescriptor) protocol {
	if descriptor.Protocol == downloader.ControlProtocolTransmissionV6 {
		return protocolJSONRPC
	}
	return protocolLegacy
}
