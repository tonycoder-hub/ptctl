...
func ensureForgetRootIntent(ctx context.Context, target *fsbind.Session, handle *journalHandle, marker ForgetIntent, maximumBytes int64) (ForgetMarkerID, forgetMarkerReceipt, fsbind.ObjectInfo, error) {
	...
	if maximumBytes <= 0 || maximumBytes > maximumForgetMarkerBytes || int64(len(raw)) > maximumBytes {
		return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget marker byte budget is insufficient", ErrPolicy)
	}
	...
	if readErr == nil {
		if existingID != id || !sameForgetIntent(existing, marker) || !bytes.Equal(existingRaw, raw) {
			return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget root intent disagrees", ErrIntegrity)
		}
		...
		if handle != nil && handle.subtree != nil {
			...
			if pendingErr == nil {
				if pending.Kind != fsbind.ObjectKindRegular || pending.SizeBytes != int64(len(raw)) {
					return id, receipt, object, fmt.Errorf("%w: client adoption forget pending marker is unsafe", ErrIntegrity)
				}
				if identity, verifyErr := handle.verifyNamedBytes(ctx, []string{retentionDirectoryName, pendingName}, raw); verifyErr != nil || !identity.Equal(pending.Identity) {
					...
					return id, receipt, object, fmt.Errorf("%w: client adoption forget pending marker identity changed", ErrIntegrity)
				}
	...
	if handle == nil || handle.subtree == nil {
		return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption retention tombstone is unavailable before forget intent publication", ErrIntegrity)
	}
	...
	if errors.Is(inspectErr, fsbind.ErrNotFound) {
		...
		if closeErr != nil {
			return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("close client adoption forget pending marker failed")
		}
		if info.SizeBytes != int64(len(raw)) || info.Identity.IsZero() {
			return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget pending marker changed", ErrIntegrity)
		}
		...
		return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget pending marker is unsafe", ErrIntegrity)
	}
	if identity, verifyErr := handle.verifyNamedBytes(ctx, []string{retentionDirectoryName, pendingName}, raw); verifyErr != nil || !identity.Equal(pending.Identity) {
		if verifyErr != nil {
			return "", receipt, fsbind.ObjectInfo{}, verifyErr
		}
		return "", receipt, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget pending marker identity changed", ErrIntegrity)
	}
	...
	if freshErr != nil || freshID != id || !sameForgetIntent(fresh, marker) || !bytes.Equal(freshRaw, raw) || !object.Identity.Equal(pending.Identity) {
		...
		return id, receipt, object, fmt.Errorf("%w: published client adoption forget intent changed", ErrIntegrity)
	}
	if err := target.SyncRoot(ctx); err != nil {
		return id, receipt, object, err
	}
...
func readForgetRootIntent(ctx context.Context, target *fsbind.Session, name string, limit int64) (ForgetIntent, ForgetMarkerID, fsbind.ObjectInfo, []byte, error) {
	if target == nil || limit <= 0 || limit > maximumForgetMarkerBytes {
		return ForgetIntent{}, "", fsbind.ObjectInfo{}, nil, fmt.Errorf("%w: client adoption forget marker read is invalid", ErrPolicy)
	}
	readOnce := func() ([]byte, fsbind.ObjectInfo, error) {
		...
		if before.SizeBytes < 0 || before.SizeBytes > maximumForgetMarkerBytes {
			_ = file.Close()
			return nil, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget marker size is invalid", ErrIntegrity)
		}
		if before.SizeBytes > limit {
			_ = file.Close()
			return nil, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget marker byte budget was reached", ErrPolicy)
		}
		...
		if closeErr != nil {
			return nil, fsbind.ObjectInfo{}, fmt.Errorf("close client adoption forget marker failed")
		}
		if int64(len(raw)) != before.SizeBytes || int64(len(raw)) > limit || !before.Identity.Equal(after.Identity) ||
			...
			return nil, fsbind.ObjectInfo{}, fmt.Errorf("%w: client adoption forget marker changed while read", ErrIntegrity)
		}
	...
	if !object.Identity.Equal(freshObject.Identity) || object.SizeBytes != freshObject.SizeBytes || !bytes.Equal(raw, freshRaw) {
		return ForgetIntent{}, "", fsbind.ObjectInfo{}, nil, fmt.Errorf("%w: client adoption forget marker name changed", ErrIntegrity)
	}
	...
	for {
		...
		if count > 0 {
			...
			if int64(buffer.Len()) > limit {
				return nil, fmt.Errorf("%w: client adoption forget marker byte budget was reached", ErrPolicy)
			}
		}
		if err == io.EOF {
