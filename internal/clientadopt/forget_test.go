...
func (fixture terminalAdoptionFixture) forgetOptions() ForgetOptions {
	return ForgetOptions{TargetRoot: fixture.materialized.targetRoot, OperationID: fixture.prepared.OperationID(),
		ExpectedPlanID: fixture.prepared.PlanID(), Acknowledge: true, Limits: DefaultForgetLimits()}
}

func TestForgetAdoptionTombstoneRemovesOnlyExactHistoricalAuthority(t *testing.T) {
	fixture := makeTerminalAdoptionFixture(t)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
	verified, before, err := VerifyCompletion(context.Background(), CompletionProofOptions{TargetRoot: fixture.materialized.targetRoot,
	...
	if forgetErr != nil || report.Outcome != ForgetOutcomeForgotten || report.WritesPerformed == 0 || report.WritesUncertain ||
		...
		report.Proof.AttemptsRecorded != 1 || report.Proof.CompletionID == "" {
		t.Fatalf("forget=%#v err=%v", report, forgetErr)
	}
	operationName, _ := operationDirectoryName(fixture.prepared.OperationID())
	rootName, _ := ForgetRootName(fixture.prepared.OperationID())
	for _, name := range []string{operationName, rootName} {
		if _, statErr := os.Stat(filepath.Join(fixture.materialized.targetRoot, name)); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("forgotten control object %q remains: %v", name, statErr)
		}
	}
	if _, _, verifyErr := VerifyCompletion(context.Background(), CompletionProofOptions{TargetRoot: fixture.materialized.targetRoot,
		OperationID: fixture.prepared.OperationID(), ExpectedPlanID: fixture.prepared.PlanID()}); !errors.Is(verifyErr, ErrOperationNotFound) {
		t.Fatalf("forgotten adoption still granted historical authority: %v", verifyErr)
	}
...
func TestForgetAdoptionCrashBoundariesRecoverOnlyThroughExplicitForget(t *testing.T) {
	for _, phase := range []string{"pending_staged", "root_intent_published", "operation_removed"} {
		t.Run(phase, func(t *testing.T) {
			...
			stop := errors.New("stop adoption forget")
			...
			if granted, _, verifyErr := VerifyCompletion(context.Background(), CompletionProofOptions{TargetRoot: fixture.materialized.targetRoot,
				OperationID: fixture.prepared.OperationID(), ExpectedPlanID: fixture.prepared.PlanID()}); granted != nil || !errors.Is(verifyErr, ErrPolicy) {
				t.Fatalf("forget boundary granted authority: %#v %v", granted, verifyErr)
			}
			if pruned, pruneErr := Prune(context.Background(), fixture.pruneOptions()); !errors.Is(pruneErr, ErrPolicy) || pruned.Outcome != RetentionOutcomeBlocked {
			...
			resumed, resumeErr := Resume(context.Background(), fixture.prepared.OperationID(), RunOptions{Prepared: fixture.prepared,
				ExpectedPlanID: fixture.prepared.PlanID(), Metafile: fixture.materialized.payload(t), Session: session})
			if !errors.Is(resumeErr, ErrPolicy) || resumed.Outcome != OutcomeBlocked || session.requests != 1 || session.adds != 0 {
				t.Fatalf("resume crossed forget boundary: %#v requests=%d adds=%d err=%v", resumed, session.requests, session.adds, resumeErr)
			}
...
func TestForgetAdoptionRejectsPolicyBeforeWrites(t *testing.T) {
	fixture := makeTerminalAdoptionFixture(t)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
	operationName, _ := operationDirectoryName(fixture.prepared.OperationID())
	rootName, _ := ForgetRootName(fixture.prepared.OperationID())
	wrongPlan := fixture.prepared.PlanID()[:len(fixture.prepared.PlanID())-1] + map[bool]string{true: "1", false: "0"}[fixture.prepared.PlanID()[len(fixture.prepared.PlanID())-1] == '0']
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, statErr := os.Stat(filepath.Join(fixture.materialized.targetRoot, operationName)); statErr != nil {
				t.Fatalf("invalid request changed tombstone: %v", statErr)
			}
			if _, statErr := os.Stat(filepath.Join(fixture.materialized.targetRoot, rootName)); !errors.Is(statErr, os.ErrNotExist) {
...
func TestForgetAdoptionRequiresExactRetainedNamespace(t *testing.T) {
	fixture := makeTerminalAdoptionFixture(t)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
	operationName, _ := operationDirectoryName(fixture.prepared.OperationID())
	if err := os.WriteFile(filepath.Join(fixture.materialized.targetRoot, operationName, "unexpected.bin"), []byte("unexpected"), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Forget(context.Background(), fixture.forgetOptions())
...
func TestForgetAdoptionRebindsRootIntentBeforeDeletingTombstone(t *testing.T) {
	fixture := makeTerminalAdoptionFixture(t)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
	...
	forgetTransitionHook = func(phase string) error {
		if phase == "root_intent_published" {
			return os.WriteFile(filepath.Join(fixture.materialized.targetRoot, rootName), []byte("{}\n"), 0o600)
		}
		return nil
	}
	t.Cleanup(func() { forgetTransitionHook = nil })
	...
	for _, path := range []string{
		filepath.Join(fixture.materialized.targetRoot, operationName),
		filepath.Join(fixture.materialized.targetRoot, operationName, retentionDirectoryName, retentionIntentName),
		filepath.Join(fixture.materialized.targetRoot, operationName, retentionDirectoryName, retentionCompleteName),
	} {
...
func TestAdoptionForgetMarkerCanonicalAndReserved(t *testing.T) {
	fixture := makeTerminalAdoptionFixture(t)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
	handle, _, err := openJournal(context.Background(), fixture.materialized.targetRoot, fixture.prepared.OperationID(), false, nil)
	if err != nil {
		t.Fatal(err)
	}
	state := handle.retention
	marker := ForgetIntent{Schema: ForgetIntentSchemaV1, OperationID: fixture.prepared.OperationID(), OperationRootIdentity: handle.subtree.Identity().String(),
		TargetRootIdentity: handle.session.Info().Identity.String(), PlanID: fixture.prepared.PlanID(), RetentionIntentMarkerID: state.IntentID,
	...
	changed := marker
	changed.RetentionComplete.CompletionID = MarkerID("sha256:" + strings.Repeat("f", 64))
	if _, _, err := EncodeForgetIntent(changed); err == nil {
		t.Fatal("forget marker with a contradictory completion link was accepted")
	}
	operationName, _ := operationDirectoryName(fixture.prepared.OperationID())
	rootName, err := ForgetRootName(fixture.prepared.OperationID())
	if err != nil || !strings.HasPrefix(rootName, ".ptctl-client-adopt-forget-") || operationName == rootName {
