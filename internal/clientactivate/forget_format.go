...
const (
	ForgetIntentSchemaV1 = "ptctl.client-activation-forget-intent/v1"
	ForgetBasisExact     = "exact_activation_retention_tombstone_selected_for_erasure"

	forgetIntentDigestDomain = "ptctl-client-activation-forget-intent-v1\x00"
...
func ParseForgetMarkerID(value string) (ForgetMarkerID, error) {
	if !canonicalSHA256ID(value) {
		return "", fmt.Errorf("invalid client activation forget marker ID")
	}
...
// exact client-activation retention tombstone. It is published before the
...
func (marker ForgetIntent) Validate() error {
	if marker.Schema != ForgetIntentSchemaV1 || marker.Basis != ForgetBasisExact || !canonicalPlanID(marker.PlanID) ||
		...
		return fmt.Errorf("%w: client activation forget intent is invalid", ErrIntegrity)
	}
	if parsed, err := ParseOperationID(marker.OperationID.String()); err != nil || parsed != marker.OperationID {
		return fmt.Errorf("%w: client activation forget operation is invalid", ErrIntegrity)
	}
	if parsed, err := ParseRetentionMarkerID(marker.RetentionIntentMarkerID.String()); err != nil || parsed != marker.RetentionIntentMarkerID {
		return fmt.Errorf("%w: client activation forget retention intent link is invalid", ErrIntegrity)
	}
	if parsed, err := ParseRetentionMarkerID(marker.RetentionCompleteMarkerID.String()); err != nil || parsed != marker.RetentionCompleteMarkerID {
		return fmt.Errorf("%w: client activation forget retention completion link is invalid", ErrIntegrity)
	}
	...
	if intentErr != nil || completeErr != nil || len(intentRaw) == 0 || len(completeRaw) == 0 ||
		...
		return fmt.Errorf("%w: client activation forget retained tombstone link is invalid", ErrIntegrity)
	}
	...
	if operationErr != nil || targetErr != nil || operationRoot.IsZero() || targetRoot.IsZero() {
		return fmt.Errorf("%w: client activation forget filesystem identity is invalid", ErrIntegrity)
	}
	return nil
}
...
func EncodeForgetIntent(marker ForgetIntent) ([]byte, ForgetMarkerID, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("encode client activation forget marker failed")
	}
	raw = append(raw, '\n')
	if int64(len(raw)) > maximumForgetMarkerBytes {
		return nil, "", fmt.Errorf("%w: client activation forget marker byte limit exceeded", ErrPolicy)
	}
...
func DecodeForgetIntent(reader io.Reader) (ForgetIntent, ForgetMarkerID, error) {
	...
	if reader == nil {
		return marker, "", fmt.Errorf("%w: client activation forget marker reader is unavailable", ErrIntegrity)
	}
	...
	if int64(len(raw)) > maximumForgetMarkerBytes || len(raw) == 0 || raw[len(raw)-1] != '\n' || !json.Valid(raw) {
		return marker, "", fmt.Errorf("%w: client activation forget marker is not canonical", ErrIntegrity)
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&marker); err != nil {
		return ForgetIntent{}, "", fmt.Errorf("%w: client activation forget marker is not canonical", ErrIntegrity)
	}
	var trailing any
...
func ForgetRootName(operation OperationID) (string, error) {
	...
	return materialize.ClientActivateForgetMarkerPrefix + strings.TrimPrefix(operation.String(), "sha256:") + forgetRootSuffix, nil
}

func ParseForgetRootName(name string) (OperationID, error) {
	if !strings.HasPrefix(name, materialize.ClientActivateForgetMarkerPrefix) || !strings.HasSuffix(name, forgetRootSuffix) {
		return "", fmt.Errorf("invalid client activation forget marker name")
	}
	hexDigest := strings.TrimSuffix(strings.TrimPrefix(name, materialize.ClientActivateForgetMarkerPrefix), forgetRootSuffix)
	if len(hexDigest) != 64 {
		return "", fmt.Errorf("invalid client activation forget marker name")
	}
