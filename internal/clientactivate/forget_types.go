...
func (limits ForgetLimits) Validate() error {
	if limits.MaxMarkerBytes <= 0 || limits.MaxMarkerBytes > maximumForgetMarkerBytes {
		return fmt.Errorf("%w: client activation forget limits are invalid", ErrPolicy)
	}
	return nil
}
...
func newForgetReport(options ForgetOptions) ForgetReport {
	return ForgetReport{
		...
		Effect: []string{"read_private_client_activation_tombstone", "write_private_client_activation_forget_intent",
			"delete_private_client_activation_tombstone", "delete_private_client_activation_forget_intent"},
		...
		Warnings: []string{
			"forget irreversibly removes one retained client-activation tombstone and its final historical attribution",
			"materialized bytes, downloader state, source names, and every other operation remain outside this authority",
			"forget cannot revoke a process-local VerifiedCompletion authority already issued before this invocation",
