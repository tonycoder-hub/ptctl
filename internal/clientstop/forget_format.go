...
const (
	ForgetIntentSchemaV1 = "ptctl.client-stop-forget-intent/v1"
	ForgetBasisExact     = "exact_stop_retention_tombstone_selected_for_erasure"

	forgetIntentDigestDomain = "ptctl-client-stop-forget-intent-v1\x00"
...
func ParseForgetMarkerID(value string) (ForgetMarkerID, error) {
	if !canonicalSHA256ID(value) {
		return "", fmt.Errorf("invalid client stop forget marker ID")
	}
...
// exact client-stop retention tombstone. It is published before the
...
func (marker ForgetIntent) Validate() error {
	if marker.Schema != ForgetIntentSchemaV1 || marker.Basis != ForgetBasisExact || !canonicalPlanID(marker.PlanID) ||
		...
		return fmt.Errorf("%w: client stop forget intent is invalid", ErrIntegrity)
	}
	if parsed, err := ParseOperationID(marker.OperationID.String()); err != nil || parsed != marker.OperationID {
		return fmt.Errorf("%w: client stop forget operation is invalid", ErrIntegrity)
	}
	if parsed, err := ParseRetentionMarkerID(marker.RetentionIntentMarkerID.String()); err != nil || parsed != marker.RetentionIntentMarkerID {
		return fmt.Errorf("%w: client stop forget retention intent link is invalid", ErrIntegrity)
	}
	if parsed, err := ParseRetentionMarkerID(marker.RetentionCompleteMarkerID.String()); err != nil || parsed != marker.RetentionCompleteMarkerID {
		return fmt.Errorf("%w: client stop forget retention completion link is invalid", ErrIntegrity)
	}
	...
	if intentErr != nil || completeErr != nil || len(intentRaw) == 0 || len(completeRaw) == 0 ||
		...
		return fmt.Errorf("%w: client stop forget retained tombstone link is invalid", ErrIntegrity)
	}
	...
	if operationErr != nil || targetErr != nil || operationRoot.IsZero() || targetRoot.IsZero() {
		return fmt.Errorf("%w: client stop forget filesystem identity is invalid", ErrIntegrity)
	}
	return nil
}
...
func EncodeForgetIntent(marker ForgetIntent) ([]byte, ForgetMarkerID, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("encode client stop forget marker failed")
	}
	raw = append(raw, '\n')
	if int64(len(raw)) > maximumForgetMarkerBytes {
		return nil, "", fmt.Errorf("%w: client stop forget marker byte limit exceeded", ErrPolicy)
	}
...
func DecodeForgetIntent(reader io.Reader) (ForgetIntent, ForgetMarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return ForgetIntent{}, "", fmt.Errorf("%w: client stop forget marker is not canonical", ErrIntegrity)
	}
...
func ForgetRootName(operation OperationID) (string, error) {
	...
	return materialize.ClientStopForgetMarkerPrefix + strings.TrimPrefix(operation.String(), "sha256:") + forgetRootSuffix, nil
}

func ParseForgetRootName(name string) (OperationID, error) {
	if !strings.HasPrefix(name, materialize.ClientStopForgetMarkerPrefix) || !strings.HasSuffix(name, forgetRootSuffix) {
		return "", fmt.Errorf("invalid client stop forget marker name")
	}
	hexDigest := strings.TrimSuffix(strings.TrimPrefix(name, materialize.ClientStopForgetMarkerPrefix), forgetRootSuffix)
	if len(hexDigest) != 64 {
		return "", fmt.Errorf("invalid client stop forget marker name")
	}
