...
type terminalStopFixture struct {
	root         string
	plan         Plan
	planID       string
...
func makeTerminalStopFixture(t *testing.T, retainResponse bool) terminalStopFixture {
	t.Helper()
	...
	completion := testStopCompletion(operation, planID, plan, attemptID, responseID, now.Add(2*time.Second), retainResponse)
	...
	return terminalStopFixture{root: root, plan: plan, planID: planID, operation: operation,
		intentID: intentID, attemptID: attemptID, responseID: responseID, completionID: completionID}
}

func (fixture terminalStopFixture) pruneOptions() PruneOptions {
...
func (fixture terminalStopFixture) forgetOptions() ForgetOptions {
...
func TestStopPruneRetainsSparseRequestEvidenceAndIsIdempotent(t *testing.T) {
	for _, retainResponse := range []bool{true, false} {
		t.Run(map[bool]string{true: "accepted_response", false: "response_missing_after_attempt"}[retainResponse], func(t *testing.T) {
			fixture := makeTerminalStopFixture(t, retainResponse)
...
func TestStopRetentionIntentRejectsChangedResponseEvidence(t *testing.T) {
...
func TestStopPruneCrashBoundaryResumesWithoutDownloader(t *testing.T) {
	fixture := makeTerminalStopFixture(t, true)
...
func TestStopForgetErasesOnlyExplicitRetainedHistory(t *testing.T) {
	fixture := makeTerminalStopFixture(t, true)
...
func TestStopForgetCrashBoundariesRecoverFromExactRootAuthority(t *testing.T) {
...
func TestStopForgetRejectsUnexpectedTombstoneObjectsBeforeWrite(t *testing.T) {
	fixture := makeTerminalStopFixture(t, true)
	if _, err := Prune(context.Background(), fixture.pruneOptions()); err != nil {
		t.Fatal(err)
	}
...
func TestStopRetentionPolicyFailsBeforeWrite(t *testing.T) {
	fixture := makeTerminalStopFixture(t, true)
	for _, mutate := range []func(*PruneOptions){
		...
		func(options *PruneOptions) { options.OperationID = OperationIDForPlan(strings.Repeat("e", 24)) },
