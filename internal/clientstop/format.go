...
func encodeCanonical(value any) ([]byte, error) {
	...
	if int64(len(raw)) > maximumMarkerBytes {
		return nil, fmt.Errorf("client stop marker exceeds its byte limit")
	}
	return raw, nil
}
...
func encodeIntent(value Intent) ([]byte, MarkerID, error) {
	...
	return markerIdentity("ptctl-client-stop-intent-v1\x00", raw, err)
}
...
func encodeAttempt(value Attempt) ([]byte, MarkerID, error) {
	...
	raw, err := encodeCanonical(value)
	return markerIdentity("ptctl-client-stop-attempt-v1\x00", raw, err)
}
...
func encodeResponse(value Response) ([]byte, MarkerID, error) {
	...
	raw, err := encodeCanonical(value)
	return markerIdentity("ptctl-client-stop-response-v1\x00", raw, err)
}
...
func encodeCompletion(value Completion) ([]byte, MarkerID, error) {
	...
	raw, err := encodeCanonical(value)
	return markerIdentity("ptctl-client-stop-complete-v1\x00", raw, err)
}
...
func decodeIntent(reader io.Reader) (Intent, MarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return Intent{}, "", fmt.Errorf("%w: client stop intent is not canonical", ErrIntegrity)
	}
	return value, id, nil
}
...
func decodeAttempt(reader io.Reader) (Attempt, MarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return Attempt{}, "", fmt.Errorf("%w: client stop attempt is not canonical", ErrIntegrity)
	}
	return value, id, nil
}
...
func decodeResponse(reader io.Reader) (Response, MarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return Response{}, "", fmt.Errorf("%w: client stop response is not canonical", ErrIntegrity)
	}
	return value, id, nil
}
...
func decodeCompletion(reader io.Reader) (Completion, MarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return Completion{}, "", fmt.Errorf("%w: client stop completion is not canonical", ErrIntegrity)
	}
	return value, id, nil
}
...
func readCanonical(reader io.Reader, destination any) ([]byte, error) {
	if reader == nil {
		return nil, fmt.Errorf("%w: client stop marker reader is unavailable", ErrIntegrity)
	}
	...
	if int64(len(raw)) > maximumMarkerBytes || len(raw) == 0 || raw[len(raw)-1] != '\n' || bytes.Count(raw, []byte{'\n'}) != 1 || !json.Valid(raw) {
		return nil, fmt.Errorf("%w: client stop marker framing is invalid", ErrIntegrity)
	}
	duplicateDecoder := json.NewDecoder(bytes.NewReader(raw))
	budget := 768
	if err := rejectDuplicateKeys(duplicateDecoder, 0, &budget); err != nil {
		return nil, fmt.Errorf("%w: client stop marker JSON is invalid", ErrIntegrity)
	}
	var trailing any
	if err := duplicateDecoder.Decode(&trailing); !errors.Is(err, io.EOF) {
		return nil, fmt.Errorf("%w: client stop marker has trailing data", ErrIntegrity)
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(destination); err != nil {
		return nil, fmt.Errorf("%w: client stop marker schema is invalid", ErrIntegrity)
	}
