...
const (
	RetentionIntentSchemaV1   = "ptctl.client-stop-retention-intent/v1"
	RetentionCompleteSchemaV1 = "ptctl.client-stop-retention-complete/v1"
	RetentionBasisTerminal    = "terminal_stop_journal_exact"
	...
	retentionIntentDomain   = "ptctl-client-stop-retention-intent-v1\x00"
	retentionCompleteDomain = "ptctl-client-stop-retention-complete-v1\x00"
)
...
func ParseRetentionMarkerID(value string) (RetentionMarkerID, error) {
	if !canonicalSHA256ID(value) {
		return "", fmt.Errorf("invalid client stop retention marker ID")
	}
...
// RetainedResponse preserves the sparse response side of the stop
...
func (marker RetentionIntent) Validate() error {
	if marker.Schema != RetentionIntentSchemaV1 || marker.Basis != RetentionBasisTerminal ||
		...
		return fmt.Errorf("%w: client stop retention intent is invalid", ErrIntegrity)
	}
	...
	if operationErr != nil || targetErr != nil || operationRoot.IsZero() || targetRoot.IsZero() {
		return fmt.Errorf("%w: client stop retention filesystem identity is invalid", ErrIntegrity)
	}
	intentRaw, intentID, err := encodeIntent(marker.Intent)
	if err != nil || len(intentRaw) == 0 || marker.IntentID != intentID {
		return fmt.Errorf("%w: client stop retention intent link is invalid", ErrIntegrity)
	}
	...
	for index, attempt := range marker.Attempts {
		...
		if encodeErr != nil || id != marker.AttemptIDs[index] || validateNextAttempt(state, attempt) != nil {
			return fmt.Errorf("%w: client stop retention attempt chain is invalid", ErrIntegrity)
		}
	...
	for _, retained := range marker.Responses {
		if retained.Sequence <= previousSequence || retained.Sequence > len(state.Attempts) {
			return fmt.Errorf("%w: client stop retention response order is invalid", ErrIntegrity)
		}
		...
		if encodeErr != nil || id != retained.MarkerID || validateResponseForAttempt(state, retained.Sequence, retained.Response) != nil {
			return fmt.Errorf("%w: client stop retention response chain is invalid", ErrIntegrity)
		}
	...
	if err != nil || completionID != marker.CompletionID || validateCompletionForState(state, marker.Completion) != nil {
		return fmt.Errorf("%w: client stop retention completion link is invalid", ErrIntegrity)
	}
	return nil
}
...
func (marker RetentionComplete) Validate() error {
	if marker.Schema != RetentionCompleteSchemaV1 || !canonicalPlanID(marker.PlanID) {
		return fmt.Errorf("%w: client stop retention completion is invalid", ErrIntegrity)
	}
	if parsed, err := ParseOperationID(marker.OperationID.String()); err != nil || parsed != marker.OperationID {
		return fmt.Errorf("%w: client stop retention completion operation is invalid", ErrIntegrity)
	}
	if OperationIDForPlan(marker.PlanID) != marker.OperationID {
		return fmt.Errorf("%w: client stop retention completion operation disagrees with its plan", ErrIntegrity)
	}
	if parsed, err := ParseRetentionMarkerID(marker.IntentMarkerID.String()); err != nil || parsed != marker.IntentMarkerID {
		return fmt.Errorf("%w: client stop retention completion intent link is invalid", ErrIntegrity)
	}
	if parsed, err := parseMarkerID(marker.CompletionID.String()); err != nil || parsed != marker.CompletionID {
		return fmt.Errorf("%w: client stop retention completion marker link is invalid", ErrIntegrity)
	}
	operationRoot, operationErr := fsbind.ParseIdentity(marker.OperationRootIdentity)
	targetRoot, targetErr := fsbind.ParseIdentity(marker.TargetRootIdentity)
	if operationErr != nil || targetErr != nil || operationRoot.IsZero() || targetRoot.IsZero() {
		return fmt.Errorf("%w: client stop retention completion filesystem identity is invalid", ErrIntegrity)
	}
	return nil
}
...
func DecodeRetentionIntent(reader io.Reader) (RetentionIntent, RetentionMarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return RetentionIntent{}, "", fmt.Errorf("%w: client stop retention intent is not canonical", ErrIntegrity)
	}
...
func DecodeRetentionComplete(reader io.Reader) (RetentionComplete, RetentionMarkerID, error) {
	...
	if err != nil || !bytes.Equal(raw, canonical) {
		return RetentionComplete{}, "", fmt.Errorf("%w: client stop retention completion is not canonical", ErrIntegrity)
	}
	return marker, id, nil
}
