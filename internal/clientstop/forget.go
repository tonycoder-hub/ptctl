...
func (*forgetInProgressError) Error() string {
	return "client stop historical evidence deletion is in progress"
}
...
// stop tombstone. A private root-level intent is durably published first
...
func Forget(ctx context.Context, options ForgetOptions) (ForgetReport, error) {
	...
	if err := options.Limits.Validate(); err != nil {
		return forgetBlocked(&report, "limits.invalid", "client stop forget limits are invalid")
	}
	if !options.Acknowledge {
		return forgetBlocked(&report, "acknowledgement.required", "historical stop tombstone deletion requires its dedicated acknowledgement")
	}
	if options.TargetRoot == "" || !canonicalPlanID(options.ExpectedPlanID) || OperationIDForPlan(options.ExpectedPlanID) != options.OperationID {
		return forgetBlocked(&report, "forget.selector_invalid", "target root, operation ID, or reviewed stop plan ID is invalid")
	}
	if parsed, err := ParseOperationID(options.OperationID.String()); err != nil || parsed != options.OperationID {
		return forgetBlocked(&report, "forget.selector_invalid", "target root, operation ID, or reviewed stop plan ID is invalid")
	}
	if err := ctx.Err(); err != nil {
		return forgetInterrupted(&report, err, "client stop forget was interrupted before target observation")
	}
	absolute, err := filepath.Abs(options.TargetRoot)
	if err != nil {
		return forgetBlocked(&report, "target.invalid_root", "the client stop target root is invalid")
	}
	target, rootInfo, err := fsbind.BindExisting(filepath.Clean(absolute))
	if err != nil {
		return mapForgetError(&report, err, "the target root cannot provide bound stop tombstone-forget semantics")
	}
	defer target.Close()
	...
	if errors.Is(markerErr, fsbind.ErrNotFound) {
		...
		if errors.Is(openErr, fsbind.ErrNotFound) {
			...
			report.Issues = append(report.Issues, Finding{Code: "forget.absent_unattributed", Message: "neither the retained stop operation nor a durable forget intent exists; prior erasure cannot be distinguished from an unknown selector"})
			return report, ErrOperationNotFound
		}
		if openErr != nil {
			return mapForgetError(&report, openErr, "the retained client stop operation could not be opened")
		}
		...
		if loadErr != nil {
			return mapForgetError(&report, loadErr, "the retained client stop tombstone could not be loaded")
		}
		if !state.IntentPresent || !state.CompletePresent || state.IntentPending || state.CompletePending {
			return forgetBlocked(&report, "operation.prune_required", "only a complete retained client stop tombstone may be forgotten")
		}
		if state.Intent.PlanID != options.ExpectedPlanID {
			return forgetBlocked(&report, "plan.id_mismatch", "the reviewed stop plan ID does not select this tombstone")
		}
		...
		if err := marker.Validate(); err != nil {
			return mapForgetError(&report, err, "the retained client stop tombstone cannot authorize forgetting")
		}
		if err := auditInitialForgetTombstone(ctx, handle, state, marker); err != nil {
			return mapForgetError(&report, err, "the client stop tombstone is not exact before forget intent publication")
		}
		preparedRaw, preparedID, prepareErr := EncodeForgetIntent(marker)
		if prepareErr != nil {
			return mapForgetError(&report, prepareErr, "the client stop forget intent cannot be encoded")
		}
		if int64(len(preparedRaw)) > options.Limits.MaxMarkerBytes {
			return mapForgetError(&report, ErrPolicy, "the client stop forget marker byte budget is insufficient")
		}
		markerID = preparedID
		populateForgetAuthority(&report, marker, markerID)
		report.Authority.State = "exact_tombstone_observed"
		report.Operation.Status, report.Operation.PhaseBefore, report.Operation.PhaseAfter = "retained", "retained_stop_completion", "retained_stop_completion"
		...
		if publishErr != nil {
			...
			return mapForgetError(&report, publishErr, "the client stop forget intent could not be durably published")
		}
	} else if markerErr != nil {
		return mapForgetError(&report, markerErr, "the client stop forget intent could not be read")
	} else if err := selectForgetMarker(marker, options, rootInfo.Identity); err != nil {
		return mapForgetError(&report, err, "the client stop forget intent does not match the explicit selector")
	} else {
		populateForgetAuthority(&report, marker, markerID)
		...
		if err := target.SyncRoot(ctx); err != nil {
			return mapForgetError(&report, err, "the visible client stop forget intent durability could not be confirmed")
		}
		...
		if freshErr != nil || !sameForgetIntent(fresh, marker) || freshID != markerID || !freshObject.Identity.Equal(rootObject.Identity) {
			if freshErr != nil {
				return mapForgetError(&report, classifyJournalError(freshErr), "the client stop forget intent could not be re-read after durability confirmation")
			}
			return mapForgetError(&report, ErrIntegrity, "the client stop forget intent changed during durability confirmation")
		}
	...
	if markerID == "" {
		...
		if encodeErr != nil {
			return mapForgetError(&report, encodeErr, "the client stop forget intent cannot be reproduced")
		}
		markerID = derivedID
	}
	populateForgetAuthority(&report, marker, markerID)
	...
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("root_intent_published"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client stop forgetting stopped after its durable root intent")
		}
	}
	...
	if handle == nil {
		...
		if parseErr != nil {
			return mapForgetError(&report, parseErr, "the client stop forget operation identity is invalid")
		}
		...
		if errors.Is(openErr, fsbind.ErrNotFound) {
			...
			case errors.Is(inspectErr, fsbind.ErrNotFound):
				if syncErr := target.SyncRoot(ctx); syncErr != nil {
					return mapForgetError(&report, syncErr, "the recovered client stop operation absence is not durable")
				}
				if _, confirmErr := target.InspectRoot(ctx, operationName); !errors.Is(confirmErr, fsbind.ErrNotFound) {
					if confirmErr == nil {
						confirmErr = fmt.Errorf("%w: the client stop operation reappeared", ErrIntegrity)
					}
					return mapForgetError(&report, confirmErr, "the recovered client stop operation absence changed")
				}
			case inspectErr != nil:
				return mapForgetError(&report, inspectErr, "the client stop operation removal state is unavailable")
			case observed.Kind != fsbind.ObjectKindDirectory || !observed.Identity.Equal(expectedOperation):
				return mapForgetError(&report, fsbind.ErrUnsafeObject, "the client stop operation residue changed identity")
			default:
				...
				if residueErr != nil {
					return mapForgetError(&report, residueErr, "the client stop empty operation residue could not be removed")
				}
			}
		} else if openErr != nil {
			return mapForgetError(&report, openErr, "the client stop operation could not be rebound for forgetting")
		} else {
			handle = &journalHandle{session: target, subtree: subtree}
			defer func() {
				if handle != nil && handle.subtree != nil {
	...
	if handle != nil {
		...
		report.recordForgetMarker(receipt)
		if ensureErr != nil {
			return mapForgetError(&report, ensureErr, "the existing client stop forget intent could not be rebound to its retained tombstone")
		}
		if ensuredID != markerID || !ensuredObject.Identity.Equal(rootObject.Identity) {
			return mapForgetError(&report, ErrIntegrity, "the existing client stop forget intent changed while its tombstone was rebound")
		}
		rootObject = ensuredObject
		if err := removeTombstoneFromForgetIntent(ctx, target, handle, marker, &report); err != nil {
			return mapForgetError(&report, err, "the retained client stop tombstone could not be completely removed")
		}
		handle = nil
	}
	...
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("operation_removed"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client stop forgetting stopped after operation removal")
		}
	}
	...
	if freshErr != nil || !sameForgetIntent(freshMarker, marker) || freshID != markerID || !freshObject.Identity.Equal(rootObject.Identity) {
		...
		if freshErr != nil {
			return mapForgetError(&report, freshErr, "the final client stop forget marker read failed")
		}
		return mapForgetError(&report, ErrIntegrity, "the final client stop forget marker changed")
	}
	if forgetTransitionHook != nil {
		if hookErr := forgetTransitionHook("before_root_intent_remove"); hookErr != nil {
			return mapForgetError(&report, hookErr, "client stop forgetting stopped before last-marker removal")
		}
	}
	operationName, _ := operationDirectoryName(options.OperationID)
	if _, absenceErr := target.InspectRoot(ctx, operationName); !errors.Is(absenceErr, fsbind.ErrNotFound) {
		if absenceErr == nil {
			absenceErr = fmt.Errorf("%w: the client stop operation name reappeared", ErrIntegrity)
		}
		return mapForgetError(&report, absenceErr, "the client stop operation is no longer absent before last-marker removal")
	}
	if syncErr := target.SyncRoot(ctx); syncErr != nil {
		return mapForgetError(&report, syncErr, "the client stop operation absence is not durable before last-marker removal")
	}
	if _, absenceErr := target.InspectRoot(ctx, operationName); !errors.Is(absenceErr, fsbind.ErrNotFound) {
		if absenceErr == nil {
			absenceErr = fmt.Errorf("%w: the client stop operation name reappeared", ErrIntegrity)
		}
		return mapForgetError(&report, absenceErr, "the client stop operation absence changed before last-marker removal")
	}
	...
	if removeErr != nil {
		...
		return mapForgetError(&report, removeErr, "the last client stop forget marker removal was not confirmed")
	}
	if !rootRemoval.Removed || rootRemoval.Durability != fsbind.DurabilityConfirmed {
		report.Authority.State = "last_marker_removal_durability_unconfirmed"
		report.Authority.MarkerDurable = false
		return mapForgetError(&report, fsbind.ErrDurabilityUnconfirmed, "the last client stop forget marker durability was not confirmed")
	}
	...
	report.Warnings = append(report.Warnings, "forget completed: no ptctl client stop history for this operation remains in the target root")
	return report, nil
}
...
func selectForgetMarker(marker ForgetIntent, options ForgetOptions, targetIdentity fsbind.Identity) error {
	...
	if marker.OperationID != options.OperationID || marker.TargetRootIdentity != targetIdentity.String() {
		return fmt.Errorf("%w: client stop forget marker is bound to another filesystem object", ErrIntegrity)
	}
	if marker.PlanID != options.ExpectedPlanID {
		return fmt.Errorf("%w: client stop forget selector disagrees", ErrPolicy)
	}
	return nil
}
...
func inspectForgetControl(ctx context.Context, target *fsbind.Session, operation OperationID, expectedPlanID string) (*forgetInProgressError, error) {
	if target == nil {
		return nil, fmt.Errorf("%w: client stop forget inspection authority is unavailable", ErrIntegrity)
	}
	rootName, err := ForgetRootName(operation)
	if err != nil {
		return nil, fmt.Errorf("%w: client stop forget selector is invalid", ErrPolicy)
	}
	...
	if marker.OperationID != operation || marker.TargetRootIdentity != target.Info().Identity.String() {
		return nil, fmt.Errorf("%w: client stop forget marker is bound to another operation", ErrIntegrity)
	}
	if expectedPlanID != "" && marker.PlanID != expectedPlanID {
		return nil, fmt.Errorf("%w: client stop forget marker differs from the reviewed plan", ErrPolicy)
	}
...
func applyForgetControlReport(report *Report, pending *forgetInProgressError) {
	...
	report.Effect = "read_private_client_stop_forget_intent"
	phase := "forget_intent_staging"
	if pending.rootDurable {
		phase = "forget_intent_recorded"
	}
	report.Operation = OperationReport{ID: marker.OperationID.String(), Status: "forgetting", PhaseBefore: "retained_stop_completion", PhaseAfter: phase, Resumable: false}
	...
	report.Blockers = append(report.Blockers, "client_stop.forget_required")
	markerWarning := "client stop historical evidence deletion has a private staged intent; only explicit forget may publish or recover it"
	if pending.rootDurable {
		markerWarning = "client stop historical evidence is being forgotten under an explicit root-level recovery marker; read-only status does not reassert its durability"
	}
...
func populateForgetAuthority(report *ForgetReport, marker ForgetIntent, markerID ForgetMarkerID) {
	if report.Operation.PhaseBefore == "" || report.Operation.PhaseBefore == "unknown" {
		report.Operation.PhaseBefore = "retained_stop_completion"
	}
	plan := marker.RetentionIntent.Intent.Plan
	report.Plan = PlanReport{ID: marker.PlanID, ExpectedID: report.Plan.ExpectedID, Matches: report.Plan.ExpectedID == "" || report.Plan.ExpectedID == marker.PlanID, Data: plan}
	...
	report.Proof = RetentionProofReport{Basis: marker.RetentionIntent.Basis, IntentID: marker.RetentionIntent.IntentID.String(),
		...
		Assurance: "exact_stop_retention_tombstone_copied_into_durable_forget_intent"}
}
...
func auditInitialForgetTombstone(ctx context.Context, handle *journalHandle, state retentionState, marker ForgetIntent) error {
	if handle == nil || handle.subtree == nil || !state.IntentPresent || !state.CompletePresent || state.IntentPending || state.CompletePending ||
		...
		return fmt.Errorf("%w: retained client stop tombstone disagrees with forget intent", ErrIntegrity)
	}
	...
	if err != nil || !listing.Complete {
		...
		return fmt.Errorf("%w: client stop forget preflight inventory is incomplete", ErrIntegrity)
	}
	...
	for _, entry := range listing.Entries {
		...
		case pendingName:
			if entry.Kind != string(fsbind.ObjectKindRegular) {
				return fmt.Errorf("%w: client stop forget pending marker is unsafe", ErrIntegrity)
			}
			if _, verifyErr := handle.verifyNamedBytes(ctx, []string{retentionDirectoryName, pendingName}, raw); verifyErr != nil {
				return verifyErr
			}
		default:
			return fmt.Errorf("%w: client stop forget preflight contains an unexpected object", ErrIntegrity)
		}
	}
	if !seenIntent || !seenComplete {
		return fmt.Errorf("%w: retained client stop tombstone is incomplete", ErrIntegrity)
	}
...
func removeTombstoneFromForgetIntent(ctx context.Context, target *fsbind.Session, handle *journalHandle, marker ForgetIntent, report *ForgetReport) error {
	...
	if err != nil || handle == nil || handle.subtree == nil || !handle.subtree.Identity().Equal(expectedOperation) {
		return fmt.Errorf("%w: client stop operation identity changed", ErrIntegrity)
	}
	...
	if directoryErr != nil || directory.Kind != fsbind.ObjectKindDirectory {
		...
		return fmt.Errorf("%w: client stop retention directory is unsafe", ErrIntegrity)
	}
	...
	if err != nil || !listing.Complete {
		...
		return fmt.Errorf("%w: client stop forget recovery inventory is incomplete", ErrIntegrity)
	}
	for _, entry := range listing.Entries {
		if entry.Kind != string(fsbind.ObjectKindRegular) || entry.Name != retentionIntentName && entry.Name != retentionCompleteName {
			return fmt.Errorf("%w: client stop forget recovery contains an unexpected object", ErrIntegrity)
		}
	}
	...
	if completeErr != nil || intentErr != nil || completeID != marker.RetentionCompleteMarkerID || intentID != marker.RetentionIntentMarkerID {
		return fmt.Errorf("%w: retained client stop tombstone cannot be reproduced", ErrIntegrity)
	}
	for _, selected := range []struct {
		...
		if inspectErr != nil || object.Kind != fsbind.ObjectKindRegular || object.SizeBytes != int64(len(selected.raw)) {
			...
			return fmt.Errorf("%w: retained client stop marker is unsafe", ErrIntegrity)
		}
		...
		if verifyErr != nil || !identity.Equal(object.Identity) {
			...
			return fmt.Errorf("%w: retained client stop marker identity changed", ErrIntegrity)
		}
	...
	if err != nil || !after.Complete || len(after.Entries) != 0 {
		...
		return fmt.Errorf("%w: client stop retention directory is not empty after marker removal", ErrIntegrity)
	}
...
func removeForgetOperationRoot(ctx context.Context, target *fsbind.Session, handle *journalHandle, report *ForgetReport) error {
	...
	if err != nil || !root.Complete || len(root.Entries) != 1 || root.Entries[0].Name != operationLockEntryName || root.Entries[0].Kind != string(fsbind.ObjectKindRegular) {
		...
		return fmt.Errorf("%w: client stop operation root is not an exact lock-only namespace", ErrIntegrity)
	}
