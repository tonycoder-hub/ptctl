...
func (limits RetentionLimits) Validate() error {
	if limits.MaxMarkerBytes <= 0 || limits.MaxMarkerBytes > maximumMarkerBytes ||
		...
		return fmt.Errorf("%w: client stop retention limits are invalid", ErrPolicy)
	}
	return nil
}
...
func newRetentionReport(options PruneOptions) RetentionReport {
	return RetentionReport{
		...
		Effect: []string{
			"read_private_client_stop_operation_state",
			"write_private_client_stop_retention_markers",
			"delete_private_client_stop_operation_state",
		},
		...
		Blockers:  []Finding{}, Issues: []Finding{}, Warnings: []string{
			"prune deletes only one explicit stop operation's owner-private journal and retains a small tombstone",
