...
// Prune replaces one explicitly selected terminal stop journal with a
...
func Prune(ctx context.Context, options PruneOptions) (RetentionReport, error) {
	...
	if err := ctx.Err(); err != nil {
		return finish(err, "client stop pruning was cancelled before observation")
	}
	if options.TargetRoot == "" || options.Limits.Validate() != nil {
		report.addRetentionBlocker("limits.or_selector_invalid", "client stop prune requires a target and valid hard limits")
		return finish(fmt.Errorf("%w: client stop prune input is invalid", ErrPolicy), "client stop prune input is invalid")
	}
	if parsed, err := ParseOperationID(options.OperationID.String()); err != nil || parsed != options.OperationID ||
		...
		report.addRetentionBlocker("operation.selector_invalid", "client stop prune requires canonical operation and plan IDs")
		return finish(fmt.Errorf("%w: client stop prune selector is invalid", ErrPolicy), "client stop prune selector is invalid")
	}
	if !options.Acknowledge {
		report.addRetentionBlocker("acknowledgement.operation_state_deletion_required", "client stop prune requires explicit acknowledgement of private operation-state deletion")
		return finish(fmt.Errorf("%w: client stop prune acknowledgement is required", ErrPolicy), "client stop prune acknowledgement is required")
	}
	...
	if pending, inspectErr := inspectForgetControl(ctx, target, options.OperationID, options.ExpectedPlanID); inspectErr != nil {
		return finish(inspectErr, "the client stop forget boundary could not be inspected")
	} else if pending != nil {
		return finish(pending, "client stop historical evidence deletion is already in progress")
	}
	...
	if errors.Is(err, fsbind.ErrNotFound) {
		return finish(ErrOperationNotFound, "the explicit client stop operation does not exist")
	}
	if err != nil || object.Kind != fsbind.ObjectKindDirectory {
		if err == nil {
			err = ErrIntegrity
		}
		return finish(classifyJournalError(err), "the client stop operation directory is unsafe")
	}
	subtree, err := target.OpenPrivateSubtreeObserved(directoryName)
	if err != nil {
		return finish(classifyJournalError(err), "the client stop operation could not be bound")
	}
	...
	if err != nil {
		return finish(err, "client stop retention state could not be loaded")
	}
	if state.ForgetPending {
		return finish(&forgetInProgressError{marker: state.ForgetIntent, markerID: state.ForgetID}, "client stop historical evidence deletion is already staged")
	}
	...
	if needLive {
		...
		if err != nil {
			return finish(err, "terminal client stop journal authority is unavailable")
		}
		if live.Pending != "" || live.Completion == nil || live.CompletionID == "" {
			report.addRetentionBlocker("operation.not_terminal", "only a stop operation with a canonical terminal completion may be pruned")
			return finish(fmt.Errorf("%w: client stop operation is not terminal", ErrPolicy), "the client stop operation is not terminal")
		}
		marker, err = prepareRetentionIntent(handle, live, options, rootInfo.Identity)
		if err != nil {
			return finish(err, "the terminal stop journal could not authorize pruning")
		}
	...
	if needLive {
		if err := validateRetentionAgainstLive(marker, live); err != nil {
			return finish(err, "the retention marker disagrees with the terminal stop journal")
		}
	}
	usage, err := auditRetentionNamespace(ctx, handle, marker, options.Limits, state.DirectoryPresent, state.IntentPresent, false)
	report.Used = usage
	if err != nil {
		return finish(err, "client stop private state could not be completely inventoried")
	}
	...
	if err != nil || int64(len(intentRaw)) > options.Limits.MaxMarkerBytes {
		...
		return finish(err, "the client stop retention intent cannot be encoded within limits")
	}
	if needLive {
		if err := handle.confirmDurability(ctx); err != nil {
			return finish(err, "the terminal stop journal durability could not be refreshed")
		}
		freshUsage, auditErr := auditRetentionNamespace(ctx, handle, marker, options.Limits, state.DirectoryPresent, false, false)
		if auditErr != nil {
			return finish(auditErr, "the terminal stop journal changed before retention intent publication")
		}
	...
	if markerReceiptErr != nil {
		...
		return finish(markerReceiptErr, "client stop retention intent publication failed")
	}
	...
	if retentionTransitionHook != nil && !state.CompletePresent {
		if hookErr := retentionTransitionHook("intent_published"); hookErr != nil {
			return finish(hookErr, "client stop pruning stopped after its durable intent")
		}
	}
	if state.CompletePresent {
		...
		if err := verifyRetentionTombstone(ctx, handle, options.OperationID, rootInfo.Identity, options.Limits); err != nil {
			return finish(err, "the retained client stop tombstone is not exact")
		}
		state, err = loadRetentionState(ctx, handle, options.OperationID, rootInfo.Identity)
		if err != nil {
			return finish(err, "the retained client stop tombstone changed")
		}
	...
	if err := removeLegacyStopState(ctx, handle, marker, &report); err != nil {
		return finish(err, "client stop private journal deletion was interrupted")
	}
	if _, err := auditRetentionNamespace(ctx, handle, marker, options.Limits, true, true, true); err != nil {
		return finish(err, "client stop private journal remains after pruning")
	}
	if retentionTransitionHook != nil {
		if hookErr := retentionTransitionHook("legacy_stop_state_removed"); hookErr != nil {
			return finish(hookErr, "client stop pruning stopped after private state removal")
		}
	}
	state, err = reloadSelectedRetentionIntent(ctx, handle, options.OperationID, rootInfo.Identity, marker, intentMarkerID)
	...
	if err != nil || int64(len(completeRaw)) > options.Limits.MaxMarkerBytes {
		...
		return finish(err, "the client stop retention completion cannot be encoded")
	}
	completeReceipt, completeErr := ensureRetentionMarker(ctx, handle, retentionCompleteName, retentionCompletePending, completeRaw)
	report.recordRetentionMarker(completeReceipt)
	if completeErr != nil {
		return finish(completeErr, "client stop retention completion publication failed")
	}
	if err := verifyRetentionTombstone(ctx, handle, options.OperationID, rootInfo.Identity, options.Limits); err != nil {
		return finish(err, "the client stop retention tombstone could not be reverified")
	}
	state, err = loadRetentionState(ctx, handle, options.OperationID, rootInfo.Identity)
	if err != nil || state.CompleteID != completeID {
		...
		return finish(err, "the client stop retention completion changed")
	}
	populateRetentionCompleteReport(&report, state)
	report.Outcome = RetentionOutcomePruned
	finalizeRetentionReport(&report)
...
func reloadSelectedRetentionIntent(ctx context.Context, handle *journalHandle, operation OperationID, targetIdentity fsbind.Identity, marker RetentionIntent, markerID RetentionMarkerID) (retentionState, error) {
	...
	if !state.IntentPresent || state.IntentPending || state.IntentID != markerID || !sameRetentionIntent(state.Intent, marker) {
		return state, fmt.Errorf("%w: selected client stop retention intent changed", ErrIntegrity)
	}
	return state, nil
}
...
func prepareRetentionIntent(handle *journalHandle, live journalState, options PruneOptions, targetIdentity fsbind.Identity) (RetentionIntent, error) {
	if handle == nil || handle.subtree == nil || live.Completion == nil || live.IntentID == "" || live.CompletionID == "" ||
		...
		return RetentionIntent{}, fmt.Errorf("%w: terminal client stop authority disagrees with the selector", ErrPolicy)
	}
...
func validateRetentionAgainstLive(marker RetentionIntent, live journalState) error {
	if live.Intent != marker.Intent || live.IntentID != marker.IntentID || live.Completion == nil || *live.Completion != marker.Completion ||
		...
		return fmt.Errorf("%w: retention intent differs from terminal client stop journal", ErrIntegrity)
	}
	for index := range live.Attempts {
		if live.Attempts[index] != marker.Attempts[index] || live.AttemptIDs[index] != marker.AttemptIDs[index] {
			return fmt.Errorf("%w: retention attempt chain differs from terminal client stop journal", ErrIntegrity)
		}
	}
	for _, retained := range marker.Responses {
		if response, ok := live.Responses[retained.Sequence]; !ok || response != retained.Response || live.ResponseIDs[retained.Sequence] != retained.MarkerID {
			return fmt.Errorf("%w: retention response chain differs from terminal client stop journal", ErrIntegrity)
		}
	}
	return nil
}
...
func selectRetentionMarker(report *RetentionReport, marker RetentionIntent, options PruneOptions, targetIdentity, operationIdentity fsbind.Identity) error {
	if marker.Validate() != nil || marker.OperationID != options.OperationID || marker.PlanID != options.ExpectedPlanID ||
		...
		return fmt.Errorf("%w: client stop retention selector disagrees", ErrPolicy)
	}
	plan := marker.Intent.Plan
	report.Operation = OperationReport{ID: marker.OperationID.String(), Status: "pruning", PhaseBefore: "stop_complete", PhaseAfter: "retention_intent_recorded", Resumable: false}
...
func auditRetentionNamespace(ctx context.Context, handle *journalHandle, marker RetentionIntent, limits RetentionLimits, retentionExpected, allowMissingLegacy, requireHeavyAbsent bool) (RetentionUsage, error) {
	...
	if !listing.Complete {
		return usage, fmt.Errorf("%w: client stop operation inventory is incomplete", ErrPolicy)
	}
	...
	if usage.ObjectsConsidered > limits.MaxObjects || usage.PathBytesConsidered > limits.MaxPathBytes {
		return usage, fmt.Errorf("%w: client stop retention inventory budget is exhausted", ErrPolicy)
	}
	...
	for _, entry := range listing.Entries {
		...
		case entry.Kind == string(fsbind.ObjectKindRegular) && expected[entry.Name] != nil:
			...
			if readErr != nil || !bytesEqual(raw, expected[entry.Name]) {
				return usage, fmt.Errorf("%w: client stop legacy marker differs from retained authority", ErrIntegrity)
			}
			usage.BytesConsidered += int64(len(raw))
		default:
			return usage, fmt.Errorf("%w: client stop operation contains an unexpected object", ErrIntegrity)
		}
	}
	if !seenLock || retentionExpected && !seenRetention || !retentionExpected && seenRetention {
		return usage, fmt.Errorf("%w: client stop retention namespace is incomplete", ErrIntegrity)
	}
	if requireHeavyAbsent {
		if seenScratch || len(seenLegacy) != 0 {
			return usage, fmt.Errorf("%w: client stop legacy journal remains after pruning", ErrIntegrity)
		}
	} else if !allowMissingLegacy {
		if !seenScratch || len(seenLegacy) != len(expected) {
			return usage, fmt.Errorf("%w: client stop terminal journal is incomplete before pruning", ErrIntegrity)
		}
	}
	if seenScratch {
		...
		if !scratchListing.Complete || len(scratchListing.Entries) != 0 {
			return usage, fmt.Errorf("%w: client stop scratch is not empty", ErrIntegrity)
		}
	}
	if usage.BytesConsidered > limits.MaxMarkerBytes*int64(2*maximumAttempts+2) {
		return usage, fmt.Errorf("%w: client stop retention byte budget is exhausted", ErrPolicy)
	}
...
func removeLegacyStopState(ctx context.Context, handle *journalHandle, marker RetentionIntent, report *RetentionReport) error {
	completionRaw, _, _ := encodeCompletion(marker.Completion)
	...
	if scratch.Kind != fsbind.ObjectKindDirectory {
		return fmt.Errorf("%w: client stop scratch object is unsafe", ErrIntegrity)
	}
	...
	if listErr != nil || !listing.Complete || len(listing.Entries) != 0 {
		...
		return fmt.Errorf("%w: client stop scratch is not empty", ErrIntegrity)
	}
...
func verifyRetentionTombstone(ctx context.Context, handle *journalHandle, operation OperationID, targetIdentity fsbind.Identity, limits RetentionLimits) error {
	...
	if !state.IntentPresent || !state.CompletePresent || state.IntentPending || state.CompletePending ||
		...
		return fmt.Errorf("%w: client stop retention tombstone is incomplete", ErrIntegrity)
	}
...
func populateRetentionCompleteReport(report *RetentionReport, state retentionState) {
	...
	report.Operation.PhaseAfter = "retained_stop_completion"
