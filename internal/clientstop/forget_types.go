...
func (limits ForgetLimits) Validate() error {
	if limits.MaxMarkerBytes <= 0 || limits.MaxMarkerBytes > maximumForgetMarkerBytes {
		return fmt.Errorf("%w: client stop forget limits are invalid", ErrPolicy)
	}
	return nil
}
...
func newForgetReport(options ForgetOptions) ForgetReport {
	return ForgetReport{
		...
		Effect: []string{"read_private_client_stop_tombstone", "write_private_client_stop_forget_intent",
			"delete_private_client_stop_tombstone", "delete_private_client_stop_forget_intent"},
		...
		Warnings: []string{
			"forget irreversibly removes one retained client-stop tombstone and its final historical attribution",
