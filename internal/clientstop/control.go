...
func Status(ctx context.Context, options StatusOptions) (Report, error) {
	...
	if handle.state.Intent.PlanID != options.ExpectedPlanID {
		err = fmt.Errorf("%w: client stop selector differs from the journal", ErrPolicy)
		report.classify(err)
		return report, err
	}
	report.Outcome = statusOutcome(handle.state)
	if report.Operation.Status == "attempt_budget_exhausted" {
...
func Resume(ctx context.Context, options ResumeOptions) (Report, error) {
	...
	if handle.state.Intent.PlanID != options.ExpectedPlanID || !expectationMatchesPlan(options.Authority.Expectation(), plan) {
		err = fmt.Errorf("%w: resume authority or selector differs from the client stop journal", ErrPolicy)
		report.classify(err)
		return report, err
	}
	notBefore := latestEffectBoundary(handle.state)
	stopped, stoppedObservation, stoppedErr := clientactivate.VerifyExpectedJobStopped(ctx, options.Authority, options.Session, notBefore)
	if stoppedErr == nil {
		if handle.state.Completion != nil {
			report.Stopped = StoppedReport{Status: "exact_typed_job_stopped", Observation: stoppedObservation}
			report.Final = stoppedObservation.Final
			report.Assurance.QueueEvidence = "fresh_same_session_exact_typed_job_stopped_with_complete_layout"
			report.Assurance.FilesystemEvidence = "fresh_exact_final_reverification_after_historical_completion"
			report.Outcome, report.Operation.Status, report.Operation.Phase, report.Operation.Resumable = OutcomeAlreadyComplete, "complete", "stop_complete", false
			return report, nil
		}
		if len(handle.state.Attempts) == 0 {
			err = fmt.Errorf("%w: exact job was already stopped before any reviewed stop request", ErrPolicy)
			report.Stopped = StoppedReport{Status: "exact_typed_job_stopped_before_attempt", Observation: stoppedObservation}
			report.classify(err)
			return report, err
		}
		attemptID, responseID, hasEffectfulAttempt := completionAttempt(handle.state)
		if !hasEffectfulAttempt {
			err = fmt.Errorf("%w: exact job stopped without any possibly effectful reviewed stop request", ErrPolicy)
			report.Stopped = StoppedReport{Status: "exact_typed_job_stopped_without_effectful_attempt", Observation: stoppedObservation}
			report.classify(err)
			return report, err
		}
		return completeStopped(ctx, handle, plan, handle.state.Intent.PlanID, attemptID, responseID,
			false, stopped, stoppedObservation, &report)
	}
	if ctx.Err() != nil || !errors.Is(stoppedErr, clientactivate.ErrPolicy) {
		report.classify(stoppedErr)
		return report, stoppedErr
	}
	if handle.state.Completion != nil {
		err = fmt.Errorf("%w: a historically stopped job is currently started or unobservable", ErrPolicy)
		report.classify(err)
		return report, err
	}
	...
	if !current.JobStarted() {
		err = fmt.Errorf("%w: live exact job is not in a complete started state", ErrPolicy)
		report.classify(err)
		return report, err
	}
	sequence := len(handle.state.Attempts) + 1
	if sequence > maximumAttempts {
		err = fmt.Errorf("%w: client stop attempt budget is exhausted", ErrPolicy)
		report.classify(err)
		return report, err
	}
	priorEffectful := effectfulAttemptPossible(handle.state)
	if !priorEffectful {
		if !options.AcknowledgeStop {
			err = fmt.Errorf("%w: client stop acknowledgement is required", ErrPolicy)
			report.classify(err)
			return report, err
		}
	} else if !options.AcknowledgeStop || !options.AcknowledgeRepeat {
		err = fmt.Errorf("%w: prior stop result is unknown; both stop and repeat acknowledgements are required", ErrRequestUnknown)
		report.classify(err)
		return report, err
	}
	descriptor, descriptorErr := options.Session.ReadExistingJobStopDescriptor(ctx)
	...
	if err != nil || prepared.planID != handle.state.Intent.PlanID {
		if err == nil {
			err = fmt.Errorf("%w: live current-use state differs from the client stop journal", ErrPolicy)
		}
		report.classify(err)
		return report, err
	}
...
func validateStatusOptions(options StatusOptions) error {
	if options.TargetRoot == "" || !canonicalPlanID(options.ExpectedPlanID) || OperationIDForPlan(options.ExpectedPlanID) != options.OperationID {
		return fmt.Errorf("%w: client stop status selector is invalid", ErrPolicy)
	}
	return nil
}
...
func validateResumeOptions(options ResumeOptions) error {
	...
	if options.Authority == nil || options.Session == nil {
		return fmt.Errorf("%w: client stop resume authority is unavailable", ErrPolicy)
	}
	if options.AcknowledgeRepeat && !options.AcknowledgeStop {
		return fmt.Errorf("%w: repeat stop acknowledgement requires the base stop acknowledgement", ErrPolicy)
	}
	return nil
}
...
func reportFromJournal(state journalState) Report {
	...
	if state.Completion != nil {
		...
		if state.Completion.Basis == "exact_stopped_after_unknown_attempt_causality_unproven" {
			report.Warnings = append(report.Warnings, "historical completion did not attribute stopped state to the stop request")
		}
	} else if len(state.Attempts) >= maximumAttempts && !effectfulAttemptPossible(state) {
		report.Outcome = OutcomeBlocked
		report.Operation.Status, report.Operation.Resumable = "attempt_budget_exhausted", false
...
func statusOutcome(state journalState) string {
	...
	if len(state.Attempts) > 0 && latestRequestResultUnknown(state) {
...
func completionAttempt(state journalState) (MarkerID, MarkerID, bool) {
	for index := len(state.Attempts) - 1; index >= 0; index-- {
		sequence := index + 1
		response, observed := state.Responses[sequence]
		if !observed {
			return state.AttemptIDs[index], "", true
		}
		if response.RequestsAttempted == 1 {
			return state.AttemptIDs[index], state.ResponseIDs[sequence], true
		}
	}
	return "", "", false
}

func latestEffectBoundary(state journalState) time.Time {
	if len(state.Attempts) == 0 {
		return time.Time{}
	}
	sequence := len(state.Attempts)
	if response, exists := state.Responses[sequence]; exists {
		return response.ObservedAtEnd
	}
	return state.Attempts[sequence-1].ObservedAtEnd
}
...
func receiptFromResponse(response Response) downloader.ExistingJobMutationReceipt {
	return downloader.ExistingJobMutationReceipt{Effect: response.Effect, ObservedAtStart: response.ObservedAtStart,
		ObservedAtEnd: response.ObservedAtEnd, Complete: response.Complete, RequestsAttempted: response.RequestsAttempted,
		AutomaticRetries: response.AutomaticRetries, RedirectsFollowed: response.RedirectsFollowed, RequestBytes: response.RequestBytes,
		RequestBytesKnown: response.RequestBytesKnown, RequestID: response.RequestID, StopReason: response.StopReason}
}
